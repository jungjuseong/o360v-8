...
import { EntityArrayResponseType, ProjectCommentService } from '../service/project-comment.service';
...
export class ProjectCommentComponent implements OnInit {
  ...
  trackId = (_index: number, item: IProjectComment): number => this.projectCommentService.getProjectCommentIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.projectComments = this.refineData(dataFromBody);
  }

  protected refineData(data: IProjectComment[]): IProjectComment[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: IProjectComment[] | null): IProjectComment[] {
    return data ?? [];
  }
  ...
  protected queryBackend(predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.projectCommentService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
