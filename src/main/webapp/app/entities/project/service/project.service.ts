...
type RestOf<T extends IProject | NewProject> = Omit<
  T,
  'fiscalYear' | 'createdDate' | 'startDate' | 'deploymentDate' | 'endDate' | 'jiraUpdate'
> & {
...
export class ProjectService {
  ...
  protected convertDateFromClient<T extends IProject | NewProject | PartialUpdateProject>(project: T): RestOf<T> {
    return {
      ...project,
      fiscalYear: project.fiscalYear?.format(DATE_FORMAT) ?? null,
      createdDate: project.createdDate?.format(DATE_FORMAT) ?? null,
      startDate: project.startDate?.format(DATE_FORMAT) ?? null,
      deploymentDate: project.deploymentDate?.format(DATE_FORMAT) ?? null,
      endDate: project.endDate?.format(DATE_FORMAT) ?? null,
      jiraUpdate: project.jiraUpdate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restProject: RestProject): IProject {
    return {
      ...restProject,
      fiscalYear: restProject.fiscalYear ? dayjs(restProject.fiscalYear) : undefined,
      ...
      deploymentDate: restProject.deploymentDate ? dayjs(restProject.deploymentDate) : undefined,
      endDate: restProject.endDate ? dayjs(restProject.endDate) : undefined,
      jiraUpdate: restProject.jiraUpdate ? dayjs(restProject.jiraUpdate) : undefined,
    };
  }
