...
import com.incite.o360v.repository.UserGroupRepository;
...
class UserGroupResourceIT {
    ...
    public static UserGroup createEntity(EntityManager em) {
        UserGroup userGroup = new UserGroup().name(DEFAULT_NAME);
        // Add required entity
    ...
    public static UserGroup createUpdatedEntity(EntityManager em) {
        UserGroup userGroup = new UserGroup().name(UPDATED_NAME);
        // Add required entity
        User user = UserResourceIT.createEntity(em);
        em.persist(user);
        em.flush();
    ...
    void getAllUserGroupsWithEagerRelationshipsIsNotEnabled() throws Exception {
        ...
        restUserGroupMockMvc.perform(get(ENTITY_API_URL + "?eagerload=false")).andExpect(status().isOk());
        verify(userGroupRepositoryMock, times(1)).findAll(any(Pageable.class));
    }
